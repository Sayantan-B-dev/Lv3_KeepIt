      ...
      <div className="animate-marquee flex whitespace-nowrap gap-6 py-4">
        {displayItems.concat(displayItems).map((note, idx) => {
          const content = (
            <div
              className={`
                inline-block w-72 p-6 rounded-2xl border transition-all duration-500 h-full
                ${isLoading || isOffline 
                  ? 'bg-black/40 border-white/10 animate-pulse' 
                  : 'bg-black/40 border-white/20 hover:border-white/40 hover:bg-black/60 hover:-translate-y-1 shadow-2xl relative'
                }
              `}
            >
              {isLoading || isOffline ? (
                ...
                <>
                  ...
                  <h3 className="text-lg font-bold text-white mb-2 line-clamp-1 group-hover:text-emerald-400 transition-colors">{note.title || 'Untitled Note'}</h3>
          ...
          if (isLoading || isOffline || !note._id) {
            return <div key={idx}>{content}</div>;
          }

          return (
            <Link key={idx} to={`/note/${note._id}`} className="block h-full">
              {content}
            </Link>
          );
        })}
